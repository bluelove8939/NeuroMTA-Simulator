...
bool DMAEngine::issue_command(Command *command_p) {
    int req_num;
    int req_latency;
...
DRAMSim3DMAEngine::DRAMSim3DMAEngine(
   ...
   _command_queue_depth(command_queue_depth)
{
    dramsim3::MemorySystem *_msys_p = new dramsim3::MemorySystem(
        ...
        std::bind(&DRAMSim3DMAEngine::read_callback, this, std::placeholders::_1),
        std::bind(&DRAMSim3DMAEngine::write_callback, this, std::placeholders::_1)
    );
...
void DRAMSim3DMAEngine::read_callback(uint64_t addr) {
...
void DRAMSim3DMAEngine::write_callback(uint64_t addr) {
...
void DRAMSim3DMAEngine::print_memsys_stats() {
    dramsim3::MemorySystem *_msys_p = (dramsim3::MemorySystem *)(this->_memory_system_p);
    _msys_p->PrintStats();
}

bool DRAMSim3DMAEngine::issue_command(Command *command_p) {    
    if (this->_suspended_commands.size() < this->_command_queue_depth) {
...
void DRAMSim3DMAEngine::tick_clock() {
    dramsim3::MemorySystem *_msys_p = (dramsim3::MemorySystem *)(this->_memory_system_p);
